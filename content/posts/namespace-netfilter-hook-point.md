...
主机内,只允许TCP控制报文(TCP报文仅有头部)向外发送,包含数据的TCP报文禁止外发.

# 实现

通过Netfilter对`NF_INET_LOCAL_OUT`进行hook,丢弃包含数据的TCP报文.

# 测试

在虚拟机进行测试.有两条策略:放行ssh;阻断portainer. 
...
问题逐渐清晰,docker通过内核命名空间实现资源隔离.

盲猜`NF_INET_LOCAL_OUT`这个hook点可能跟命名空间有关,于是把hook点换成了`NF_INET_POST_ROUTING`.
